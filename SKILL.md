...
| `collapse_row_groups` | `bool` | `False` | Start with all top-level row groups collapsed (same as **Collapse All**). Users can expand interactively; persisted state is restored on reruns. No effect with <2 row dims or when no collapsible groups exist (`show_subtotals=False` and not hierarchy). |
...
- **Start collapsed:** pass `collapse_row_groups=True` to render all groups folded on first load. Works in both layouts whenever collapsible groups exist.
